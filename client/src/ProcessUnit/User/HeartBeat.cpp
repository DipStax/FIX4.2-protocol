...
namespace pu
{
    ...
    bool HeartBeatHandler::handleTestRequest(InputType &_input)
    {
        ...
        if (reject.first) {
            Logger->log<log::Level::Info>("TestRequest | TestRequest verification failed");
    ...
    bool HeartBeatHandler::handleHeartBeat(InputType &_input)
    {
        ...
        if (reject.first) {
            Logger->log<log::Level::Info>("HeartBeat | HeartBeat verification failed");
            reject.second.set45_refSeqNum(_input.at(fix::Tag::MsqSeqNum));
    ...
    void HeartBeatHandler::heartbeatLoop(std::stop_token _st)
    {
        while (!_st.stop_requested()) {
            ...
            if (std::chrono::duration<double>(now - User::Instance().getSinceHeartBeat()).count() > PU_HEARTBEAT_TO) {
                fix::HeartBeat hb;
