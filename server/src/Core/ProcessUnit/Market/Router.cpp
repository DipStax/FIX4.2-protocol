...
namespace pu::market
{
    Router::Router(std::string _symbol, InputNetworkOutput &_tcp_output, InputOBAction &_ob_action)
        : m_symbol(_symbol), m_tcp_output(_tcp_output), m_ob_action(_ob_action), Logger(log::Manager::newLogger("Market/" + _symbol + "/Router"))
    {
    }
    ...
    {
        Logger->log<log::Level::Info>("(New Order Single) Processing message...");
        ...
        if (reject.first) {
            Logger->log<log::Level::Info>("(New Order Single) Request verification failed");
            reject.second.set45_refSeqNum(_input.Message.at(fix::Tag::MsqSeqNum));
            Logger->log<log::Level::Debug>("(Logout) Reject from (", *(_input.Client), ") moving to TCP output");
        ...
        Logger->log<log::Level::Debug>("(New Order Single) Moving to target OrderBook Container new order: (", data.order.orderId,") from: (", data.order.userId, ")");
    ...
    {
        Logger->log<log::Level::Info>("(Order Cancel Request) Processing message...");
        Context<data::OBActionInput> data(_input.Client, _input.ReceiveTime);
        std::pair<bool, fix::Reject> reject = fix::OrderCancelRequest::Verify(_input.Message);

        if (reject.first) {
            Logger->log<log::Level::Info>("(Order Cancel Request) Request verification failed");
            reject.second.set45_refSeqNum(_input.Message.at(fix::Tag::MsqSeqNum));
            Logger->log<log::Level::Debug>("(Order Cancel Request) Reject from (", *(_input.Client), ") moving to TCP output");
            m_tcp_output.append(_input.Client, _input.ReceiveTime, std::move(reject.second));
            return false;
        }
        ...
        data.order.userId = _input.Client->getUserId();
        data.type = (_input.Message.at(fix::Tag::Side) == "3") ? OrderType::Bid : OrderType::Ask;
        Logger->log<log::Level::Debug>("(New Order Single) Moving to target OrderBook Container cancel order: (", data.order.orderId,") from: (", data.order.userId, ")");
        m_ob_action.push(std::move(data));
        return true;
    }
    ...
    {
        Logger->log<log::Level::Info>("(Order Cancel Replace) Processing message...");
        Context<data::OBActionInput> data(_input.Client, _input.ReceiveTime);
        std::pair<bool, fix::Reject> reject = fix::OrderCancelReplaceRequest::Verify(_input.Message);

        if (reject.first) {
            Logger->log<log::Level::Info>("(Order Cancel Replace) Request verification failed");
            reject.second.set45_refSeqNum(_input.Message.at(fix::Tag::MsqSeqNum));
            Logger->log<log::Level::Debug>("(Order Cancel Replace) Reject from (", *(_input.Client), ") moving to TCP output");
            m_tcp_output.append(_input.Client, _input.ReceiveTime, std::move(reject.second));
            return false;
        }
        ...
        data.price = utils::to<Price>(_input.Message.at(fix::Tag::Price));
        data.type = (_input.Message.at(fix::Tag::Side) == "3") ? OrderType::Bid : OrderType::Ask;
        Logger->log<log::Level::Debug>("(New Order Replace) Moving to target OrderBook Container replace order: (", data.order.orderId,") from: (", data.order.userId, ")");
        m_ob_action.push(std::move(data));
        return true;
    }
}