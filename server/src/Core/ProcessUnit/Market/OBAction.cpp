...
namespace pu::market
{
    ...
    bool OBAction::runAdd(const InputType &_data)
    {
        ...
        Logger::Log("(New) request: ", _data.order); // todo log
        if (!m_ob.add(_data.type, _data.price, order)) {
            Logger::Log("(New) Reject: Order ID already used: ", _data.order.orderId);
        ...
        Logger::Log("(New) Order executaded sucefully: ", order, ", price: ", _data.price);
        return true;
    }
    ...
    bool OBAction::runModify(const InputType &_data)
    {
        ...
        if (!m_ob.cancel(_data.type, _data.target, false)) {
            ...
            Logger::Log("(Modify-Cancel) Reject: Order ID already exist: ", _data.target);
            m_tcp_output.append(_data.Client, _data.ReceiveTime, std::move(report));
            return false;
            ...
            Logger::Log("(Modify-Add) Reject: Order ID already exist: ", _data.order.orderId);
            m_tcp_output.append(_data.Client, _data.ReceiveTime, std::move(report));
            return false;
        }
        Logger::Log("(Modify) Order modify sucessfully: ", _data.target, " -> ", order);
        return true;
    }
    ...
    bool OBAction::runCancel(const InputType &_data)
    {
        ...
        Logger::Log("(Cancel) Request: ", _data.order.orderId);
        if (!m_ob.cancel(_data.type, _data.order.orderId)) {
            Logger::Log("(Cancel) Reject: Order ID not found: ", _data.order.orderId);
        ...
        Logger::Log("(Cancel) Successfully executed on: ", _data.order.orderId);
        return true;
    }
}