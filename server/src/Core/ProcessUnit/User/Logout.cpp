...
namespace pu::user
{
    LogoutHandler::LogoutHandler(InputNetworkOutput &_tcp_output)
        : m_tcp_output(_tcp_output), Logger(log::Manager::newLogger("Logout"))
    {
    }
    ...
    bool LogoutHandler::process(InputType &&_input)
    {
        ...
        if (reject.first) {
            ...
            Logger->log<log::Level::Info>("Client try to logout without begin connected");
        ...
        Logger->log<log::Level::Info>("Client set as logged out: (", *(_input.Client), ")");
